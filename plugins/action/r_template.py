"""
description: |
  recursively installs templates from a directory to remote host.
  ...
  the string ".j2" will be stripped from any filenames.
required arguments:
...
def _strip_end(text: str, suffix: str) -> str:
...
class ActionModule(ActionBase):
    ...
    def _run_template_action(self, src: str, dest: str, tmp, task_vars) -> dict:
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        for dirpath, dirnames, filenames in os.walk(args["src_root"]):
            for filename in filenames:
                ...
                dest_path = _strip_end(dest_path, ".j2")
        ...
        for src, dest in src_dest_tuples:
            self._update_result_from_task(self._run_template_action(src, dest, tmp, task_vars))
            if self.result["failed"]:
                return self.result
        return self.result
